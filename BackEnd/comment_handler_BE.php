...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_query($con, $insertCommentQuery)) {
        if ($role == 'org') {
            header("Location: /FrontEnd/loggedIn/organizationpage/view_blog.php?post_id=$post_id");
            exit();
        } elseif ($role == 'user') {
            header("Location: /FrontEnd/loggedIn/userpage/view_blog.php?post_id=$post_id");
            exit();
        }
    } else {
